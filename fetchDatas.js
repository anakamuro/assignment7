  ...
  if (fits.length > 0) {
    var html = `<div id="ustenMain">`
     html = fits
      .map(
        (fit, index) => `
        <h4 id="ustensSearch" class="card title m1" onclick="filterUstensils('${fit}')">${fit}&nbsp;<i id="ingre-cancel" onclick=removeUtensilsTag(${index}) class="fa fa-times-circle-o" aria-hidden="true"></i></h4>
       `
      )
      .join("");
      html+= `</div>`
    ...
    document.getElementById("ustensilssList").innerHTML = "No Utensil matches your criteria...";
  }
};

function removeUtensilsTag(index){
 ...
 */
 ...
 let arr = [1, 2, 3, 4, 5, 6, 7, 8, 9 ]
  ...
  const filterAll = async () => {
    ...
    const recipes = await res.json();
    let fits = recipes.recipes.filter((recipe) => {
      return recipe.ustensils[0] == ustensTags[0] ||
             recipe.ustensils[1] == ustensTags[0] ||
